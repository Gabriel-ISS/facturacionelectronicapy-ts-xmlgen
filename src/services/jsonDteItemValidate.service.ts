...
class JSonDteItemValidateService {
  ...
  public generateDatosItemsOperacionValidate(params: any, data: any, config: XmlGenConfig, errors: Array<string>) {
    ...
    if (data['items'] && data['items'].length > 0) {
      for (let i = 0; i < data['items'].length; i++) {
        ...
        if (constanteService.measurementUnits.filter((um) => um._id === +unidadMedida).length == 0) {
          this.validator.errors.push(
            "Unidad de Medida '" +
              unidadMedida +
              "' en data.items[" +
        ...
        if (data.tipoDocumento === 7) {
          if (!item['tolerancia']) {
            ...
            if (constanteService.merchandiseRelevances.filter((um) => um._id === +item['tolerancia']).length == 0) {
              this.validator.errors.push(
                "Tolerancia de Mercaderia '" +
                  ...
                  constanteService.merchandiseRelevances.map((a) => a._id + '-' + a.description),
              );
            }
  ...
  private generateDatosItemsOperacionIVAValidate(params: any, data: any, item: any, i: number) {
    if (constanteService.taxTreatments.filter((um) => um._id === +item['ivaTipo']).length == 0) {
      this.validator.errors.push(
        "Tipo de IVA '" +
          item['ivaTipo'] +
          "' en data.items[" +
          ...
          constanteService.taxTreatments.map((a) => a._id + '-' + a.description),
      );
    }
  ...
  private generateDatosItemsOperacionSectorAutomotoresValidate(params: any, data: any, item: any, i: number) {
    ...
    if (
      constanteService.vehicleOperationTypes.filter((um) => um._id === item['sectorAutomotor']['tipo']).length ==
      0
    ) {
      this.validator.errors.push(
        "Tipo de Operación de Venta de Automotor '" +
          ...
          constanteService.vehicleOperationTypes.map((a) => a._id + '-' + a.description),
      );
    }
    if (
      constanteService.fuelTypes.filter((um) => um._id === item['sectorAutomotor']['tipoCombustible'])
        .length == 0
    ) {
      this.validator.errors.push(
        "Tipo de Combustible '" +
          ...
          constanteService.fuelTypes.map((a) => a._id + '-' + a.description),
      );
    }
