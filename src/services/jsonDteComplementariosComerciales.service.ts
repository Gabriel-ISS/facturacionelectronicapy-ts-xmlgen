  ...
  private generateDatosCarga(params: any, data: any) {
    ...
    if (
      ...
      jsonResult['dDesUniMedTotVol'] = constanteService.measurementUnits.filter(
        (td) => td.code == data.complementarios['carga']['unidadMedidaVolumenTotal'],
      )[0]['representation'];
    }
    if (
      data.complementarios &&
    ...
    if (
      ...
      jsonResult['dDesUniMedTotPes'] = constanteService.measurementUnits.filter(
        (td) => td.code == data.complementarios['carga']['unidadMedidaPesoTotal'],
      )[0]['representation'];
    }
    ...
    if (
      ...
      jsonResult['dDesCarCarga'] = constanteService.cargoCharacteristics.filter(
        (td) => td.code == data.complementarios['carga']['caracteristicaCarga'],
      )[0]['description'];
